...
HASH_CONFIG = {"Ad Topic Line": 128, "City": 32, "Country": 16}
...
def apply_feature_engineering(df):
    ...
    if "Age" in df.columns:
        ...
        df["Age_Bucket"] = pd.cut(
            df["Age"], bins=[-np.inf, 25, 45, np.inf], labels=[0, 1, 2]
        ).astype(int)

    for col, n_feats in HASH_CONFIG.items():
        if col in df.columns:
            hasher = FeatureHasher(n_features=n_feats, input_type="string")
            hashed_data = hasher.transform(df[col].astype(str).apply(lambda x: [x]))
