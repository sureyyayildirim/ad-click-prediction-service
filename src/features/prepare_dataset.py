...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    hashed_dfs = [
        hash_column(df[col], n, col.replace(" ", "_"))
        for col, n in HASH_FEATURES.items()
        if col in df.columns
    ]
    num_df_scaled = pd.DataFrame(
        StandardScaler().fit_transform(
            df[[c for c in NUMERIC_COLS if c in df.columns]]
        ),
        columns=[c for c in NUMERIC_COLS if c in df.columns],
    )
    time_cols = ["hour", "day_of_week", "is_weekend"]
    dfs_to_concat = (
        [num_df_scaled] + hashed_dfs + [df[[c for c in time_cols if c in df.columns]]]
    )
